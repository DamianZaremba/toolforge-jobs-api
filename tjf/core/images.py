    ...
    def from_url_or_name(
        ...
        * image path with host and digest: "harbor.example.org/tool-<mytool>/<myimage>:latest@sha256:123454..."
        ...
        if project:
            ...
            tool_name = project.removeprefix("tool-")
...
def _get_hostless_url(url: str) -> str:
    ...
    if url.startswith(f"{harbor_host}/"):
        return url.removeprefix(f"{harbor_host}/")

    # the next two cases will go away soon-ish
    if url.startswith("docker-registry.tools.wmflabs.org/"):
        return url.removeprefix("docker-registry.tools.wmflabs.org/")
    if url.startswith("docker-registry.svc.toolforge.org/"):
        return url.removeprefix("docker-registry.svc.toolforge.org/")
