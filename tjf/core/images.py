...
class ImageType(str, Enum):
    ...
    def _missing_(cls, value: object) -> "ImageType | None":
        if value == "buildpack":
            return cls.BUILDSERVICE
        return None

    def use_standard_nfs(self) -> bool:
        return self != ImageType.BUILDSERVICE
...
class Image(BaseModel):
    ...
    def from_short_name_or_url(
        ...
        * buildservice image of another tool: tool-<another>/<theirimage>:latest
        ...
        tool_name = image_url_parts.project.removeprefix(
            "tool-"  # we allow tools to use other tools images
        )
        ...
        if project:
            params["type"] = ImageType.BUILDSERVICE
