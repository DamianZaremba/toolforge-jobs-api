...
def _update_storage_job_status_from_runtime(
    ...
    if not runtime_job or runtime_job.model_dump() != storage_job.model_dump():
        storage_job.status_long = f"{storage_job.status_long or ''} Runtime job is different than configured, please recreate or redeploy."
...
class Core:
    ...
    def get_jobs(self, toolname: str) -> list[AnyJob]:
        ...
        if not self.settings.enable_storage:
            LOGGER.debug(f"Not using storage to get jobs for {toolname}, disabled by config")
            return runtime_jobs

        LOGGER.debug("Using storage to get jobs")
        return final_jobs
    ...
    def _reconciliate_storage_and_runtime(
        ...
        if not runtime_job and storage_job:
            LOGGER.warning(
                f"Found a job in storage but not in runtime, recreating in runtime: {storage_job}"
            )
