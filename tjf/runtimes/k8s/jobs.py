...
# TODO: Might not be needed once we split all functions per job-type
...
def get_k8s_cronjob_object(job: ScheduledJob, default_cpu_limit: str) -> K8S_OBJECT_TYPE:
...
def get_k8s_deployment_object(job: ContinuousJob, default_cpu_limit: str) -> K8S_OBJECT_TYPE:
    labels = generate_labels(
        jobname=job.job_name,
        tool_name=job.tool_name,
        job_type=job.job_type,
...
def get_k8s_job_from_cronjob(k8s_cronjob: K8S_OBJECT_TYPE) -> K8S_OBJECT_TYPE:
    ...
    k8s_job["metadata"]["ownerReferences"] = [
        {
            "apiVersion": JOB_TYPE_TO_K8S_KIND[JobType.SCHEDULED],
            "kind": "CronJob",
...
def get_k8s_objects_by_job_name(
    job_name: str, tool_account: ToolAccount, job_type: JobType, k8s_kind: str | None = None
) -> list[dict[str, Any]]:
    if not k8s_kind:
        k8s_kind = JOB_TYPE_TO_K8S_KIND[job_type]
