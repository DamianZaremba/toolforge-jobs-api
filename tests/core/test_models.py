...
class TestWebserviceJob:
    def test_to_resolved_continuous_job_returns_expected_value_when_excluding_unset(
        self,
    ):
        ...
        gotten_continuous_job = my_job.to_resolved_continuous_job()
        assert gotten_continuous_job.model_dump(
            exclude_unset=True
        ) == expected_continuous_job.model_dump(exclude_unset=True)

    def test_to_resolved_continuous_job_returns_expected_value_when_including_unset(
        self,
    ):
        my_job = get_dummy_webservice_job()
    ...
    def test_to_resolved_continuous_job_returns_expected_value_when_setting_all_fields(
        self,
    ):
        my_job = get_dummy_webservice_job(
            replicas=3,
    ...
    def test_webservice_custom_command_appended_to_default(
        ...
        continuous_job = job.to_resolved_continuous_job()
    ...
    def test_resolve_command_buildservice_defaults_to_web(
        ...
        assert job._resolve_command(command=None, port=None) == "web"
    ...
    def test_to_resolved_continuous_job_raises_when_no_command_resolvable(
        self,
    ):
        job = get_dummy_webservice_job(
        ...
        with pytest.raises(
            ...
            job.to_resolved_continuous_job()
