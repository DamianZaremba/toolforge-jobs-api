...
class TestGetK8sHttpRouteObject:
    def test_httproute_hostnames(self):
        job = get_continuous_job_fixture_as_job(
            publish=True, port=8000, tool_name="my-tool"
        )

        httproute = get_k8s_http_route_object(
            job, default_public_domain="toolforge.org"
        )
    ...
    def test_httproute_routes_to_job_service(self):
        job = get_continuous_job_fixture_as_job(
            publish=True, port=8000, job_name="my-web-job"
        )
