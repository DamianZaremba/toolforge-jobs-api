...
def test_refresh_job_short_status_cronjob(
    ...
    user = get_fake_account(fake_k8s_cli=FakeK8sCli(), name="some-tool")
    gotten_job = get_job_from_k8s(cronjob, "cronjobs", default_cpu_limit="4000m", user=user)
    refresh_job_short_status(user, gotten_job)
